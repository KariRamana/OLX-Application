...
public class AdvertiseServiceImpl implements AdvertiseService {
	...
	static {
		advertiseDetails.add(new AdvertiseDetails(1L, "laptop sale", 54000, "Electronic goods",
				"intel core 3 Sony Vaio", "anand", "7/21/22", "7/21/22", "OPEN", "Anand Kulkarni"));
	}
	...
	public Advertise searchAdvertisementsByCriteria(String category, String toDate, Long filtercriteria,
			String fromDate) {
		for (Advertise advertise : advertises) {
			if (advertise.getId() == filtercriteria || advertise.getCategory().equals(category)
					|| advertise.getCreatedDate().equals(fromDate) || advertise.getModifiedDate().equals(toDate)) {
				return advertise;
			}
